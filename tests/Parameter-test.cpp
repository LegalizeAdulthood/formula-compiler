...
TEST(TestParameterParser, parsesExtendedSectionsFromFileEntry)
{
    ...
    ASSERT_EQ(2U, result.layers.size());
    EXPECT_EQ("layer", result.layers[0].name);
    EXPECT_EQ("formula", result.layers[1].name);
    ASSERT_EQ(3U, result.layers[1].assignments.size());
    EXPECT_EQ("p_power", result.layers[1].assignments[2].key);
    EXPECT_EQ("2", result.layers[1].assignments[2].value);
}
...
TEST(TestParameterParser, preservesRepeatedAssignmentsInOrder)
{
    ...
    ASSERT_TRUE(result.diagnostics.empty());
    ASSERT_EQ(2U, result.layers.size());
    ASSERT_EQ(4U, result.layers[1].assignments.size());
    EXPECT_EQ("index", result.layers[1].assignments[0].key);
    EXPECT_EQ("0", result.layers[1].assignments[0].value);
    EXPECT_EQ("color", result.layers[1].assignments[1].key);
    EXPECT_EQ("4278190080", result.layers[1].assignments[1].value);
    EXPECT_EQ("index", result.layers[1].assignments[2].key);
    EXPECT_EQ("1", result.layers[1].assignments[2].value);
    EXPECT_EQ("color", result.layers[1].assignments[3].key);
    EXPECT_EQ("4294967295", result.layers[1].assignments[3].value);
}
