...
Scanner::Scanner(std::string_view text, std::string filename) :
    m_text(text)
{
    m_location.filename = std::move(filename);
}

std::vector<FileEntry> Scanner::entries()
{
...
std::optional<FileEntry> Scanner::next_entry()
{
...
void Scanner::parse_header(FileEntry &entry, std::size_t header_begin_position, SourceLocation header_begin_location,
    std::size_t header_end_position) const
{
...
void Scanner::strip_trailing_bracket(std::size_t begin, std::size_t &end, std::string &value) const
{
...
void Scanner::strip_trailing_paren(std::size_t begin, std::size_t &end, std::string &value) const
{
    end = trim_end(begin, end);
    if (end <= begin || m_text[end - 1] != ')')
    {
...
std::size_t Scanner::trim_begin(std::size_t begin, std::size_t end) const
{
    while (begin < end)
    {
        if (m_text[begin] == ';')
...
std::size_t Scanner::trim_end(std::size_t begin, std::size_t end) const
{
...
SourceLocation Scanner::location_at(
    std::size_t header_begin_position, SourceLocation header_begin_location, std::size_t position) const
{
...
std::size_t Scanner::next_position(std::size_t position) const
{
...
std::size_t Scanner::skip_comment(std::size_t position, std::size_t end) const
{
...
SourceLocation Scanner::scan_body(std::string &body)
{
    while (!at_end())
    {
        if (current_char() == ';')
...
void Scanner::append_string(std::string &body)
{
    append_current_char(body);
    while (!at_end())
    {
...
void Scanner::append_current_char(std::string &body)
{
    if (current_char() == '\r')
    {
        body.push_back('\r');
...
void Scanner::skip_horizontal_whitespace()
{
